...
{
    public partial class UCBuyOrderOut : UserControl
    {
        public UCBuyOrderOut()
        {
            InitializeComponent();
        ...
        private void saveButton_Click(object sender, EventArgs e)
        {
            if (listView2.Items.Count > 0)
            {
                ...
                {
                    MessageBox.Show("请填写合计!", "提示信息...", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        sqlstring = "INSERT INTO " + BaseConfigs.GetTablePrefix + "BuyOrders(OrderID,OrderDate,SupplierName,BuyName,TotalPrice,OrderType,Descn) VALUES(@OrderID,@OrderDate,@SupplierName,@BuyName,@TotalPrice,@OrderType,@Descn)";
        ...
        {
            string retVal = InputBox.Show("修改退货数量", "请输入要修改的数量。按Enter确定，Esc退出。", listView2.SelectedItems[0].SubItems[2].Text.Trim());
            if (retVal != "")
            {
        ...
        private void UCBuyOrderOut_Load(object sender, EventArgs e)
        {
        ...
        {
            string retVal = InputBox.Show("修改退货进价", "请输入要修改的进价。按Enter确定，Esc退出。", listView2.SelectedItems[0].SubItems[3].Text.Trim());
            if (retVal != "")
            {
